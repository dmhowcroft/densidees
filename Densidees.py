...
# tag=treetagger par défaut ("treetagger" is the default value)
if "tag" not in args:
    args["tag"] = "treetagger"
tagger = args["tag"]
...
def is_link(lemma):
    result = False
    if lemma == "apparaître" or lemma == "suivre|être" or lemma == "être" or lemma == "sembler" or \
                    lemma == "devenir" or lemma == "paraître" or lemma == "rester" or lemma == "demeurer":
        result = True
    return result
...
if os.path.isfile(input_filename):
    ...
    while i < len(text):
        if (tagger == "treetagger" and ((text[i - 1]["lemma"] == "être")
                                        or (text[i - 1]["lemma"] == "suivre|être")
                                        or (text[i - 1]["lemma"] == "avoir")) and (text[i]["tag"] == "VER:pper")):
            text[i - 1]["tag"] = "VER:aux"
        if (tagger == "treetagger" and ((text[i - 2]["lemma"] == "être")
    ...
    while i < len(text):
        ...
        if (tagger == "treetagger" and (text[i - 1]["lemma"] == "celui") and (
                text[i]["lemma"] == "là" or text[i]["lemma"] == "ci")):
            text[i]["isProp"] = " "
            text[i]["rule"] = "054"
    ...
    output.writelines(
        "Densité des idées : " + str(10 * round((nbProps + 0.00000001) / (nbWords + 0.00000001), 4)) + "\n")
    if visible == 1:
        print(nbWords, "mots.")
